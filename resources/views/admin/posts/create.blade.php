    ...
    <div class="tab-content">
        ...
        @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
            ...
            <div class="tab-pane show {{($force_lang == $localeCode) ? 'active' : ''}}" id="{{$localeCode}}" role="tabpanel" aria-labelledby="{{$localeCode}}-tab">
                <form method="POST" action="{{ action('\Parfaitementweb\DocBlog\Http\Controllers\Admin\PostsController@store') }}" enctype="multipart/form-data">
                    <input type="hidden" name="lang" value="{{$localeCode}}">
                    @include('docblog::admin.posts._partials.form', ['lang' => $localeCode, 'submitText' => 'Create [' . strtoupper($localeCode).']'] )
                </form>
            </div>
