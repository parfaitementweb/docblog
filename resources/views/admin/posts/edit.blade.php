    ...
    <div class="tab-content">
        ...
        @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
            ...
            <div class="tab-pane show {{($force_lang == $localeCode) ? 'active' : ''}}" id="{{$localeCode}}" role="tabpanel" aria-labelledby="{{$localeCode}}-tab">
                <form method="POST" action="{{ action('\Parfaitementweb\DocBlog\Http\Controllers\Admin\PostsController@update', $post->id) }}" enctype="multipart/form-data">
                    <input type="hidden" name="_method" value="PATCH">
                    <input type="hidden" name="lang" value="{{$localeCode}}">
                    @include('docblog::admin.posts._partials.form', ['lang' => $localeCode, 'submitText' => 'Update [' . strtoupper($localeCode).']'] )
                </form>
            </div>

        @endforeach

        <form action="{{ action('\Parfaitementweb\DocBlog\Http\Controllers\Admin\PostsController@destroy', ['post' => $post->id]) }}" method="POST" onsubmit="return confirm('!!! Delete ENTIRE article? !!!');">
            {{ csrf_field() }}
            {{ method_field('DELETE') }}
